...
defined('__TEST_25_NEW_COORDS_LAT_') or define('__TEST_25_NEW_COORDS_LAT_', 34.235951);
defined('__TEST_25_NEW_COORDS_LNG_') or define('__TEST_25_NEW_COORDS_LNG_', -118.563670);
defined('__TEST_25_FUZZ_FACTOR__') or define('__TEST_25_FUZZ_FACTOR__', 10);
    ...
    if (isset($test_harness_instance->sdk_instance)) {
        ...
        if ($test_harness_instance->sdk_instance->valid()) {
            ...
            if (isset($record) && ($record instanceof RVP_PHP_SDK_Place)) {
                ...
                if ($record->set_fuzz_factor(__TEST_25_FUZZ_FACTOR__)) {
                    if ($record->is_fuzzy()) {
                        $test_harness_instance->write_log_entry('RECORD '.__TEST_25_ID__.' IS FUZZY', $test_count++, true);
                    } else {
                        $all_pass = false;
                        $test_harness_instance->write_log_entry('RECORD '.__TEST_25_ID__.' IS NOT FUZZY', $test_count++, false);
                    ...
                    if (isset($second_sdk_instance) && ($second_sdk_instance instanceof RVP_PHP_SDK)) {
                        $second_records = $second_sdk_instance->get_objects(__TEST_25_ID__);
                        if (isset($second_records) && is_array($second_records) && (1 == count($second_records))) {
                            $new_coords = $second_records[0]->coords();
                            if (($new_coords['latitude'] != __TEST_25_NEW_COORDS_LAT_) || ($new_coords['longitude'] != __TEST_25_NEW_COORDS_LNG_)) {
                            ...
                            $all_pass = false;
                            $test_harness_instance->write_log_entry('FAILED TO FETCH RECORD '.__TEST_25_ID__, $test_count++, false);
                            echo('<h4 style="color:red">FAILED TO FETCH RECORD '.__TEST_25_ID__.'!</h4>');
                        }
                    ...
                    $all_pass = false;
                    $test_harness_instance->write_log_entry('FAILED TO SET FUZZ FACTOR FOR RECORD '.__TEST_25_ID__, $test_count++, false);
                    echo('<h4 style="color:red">FAILED TO SET FUZZ FACTOR FOR RECORD '.__TEST_25_ID__.'!</h4>');
                }
