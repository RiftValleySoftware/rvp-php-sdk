...
*/
defined('__CSV_TEST_ACCOUNTS_FILE__') or define('__CSV_TEST_ACCOUNTS_FILE__','test-20_bmlt-accounts-1.csv');
...
function run_test_20_harness_login_tests($test_harness_instance) {
    ...
    if (isset($test_harness_instance->sdk_instance)) {
        ...
        if ($test_harness_instance->sdk_instance->valid()) {
            $test_file_loc = dirname(__FILE__).'/'.__CSV_TEST_ACCOUNTS_FILE__;
    ...
    if ($in_test_harness_instance->sdk_instance->login($in_csv_row['login_id'], $in_csv_row['password'], CO_Config::$session_timeout_in_seconds)) {
        ...
        if (isset($info) && is_array($info) && count($info) && isset($info['login'])) {
            if ($info['login']->login_id() != $in_csv_row['login_id']) {
                ...
                if (isset($editable_places) && is_array($editable_places) && count($editable_places) && (count($editable_places) != intval($in_csv_row['editable_record_count']))) {
                    $all_pass = false;
